...
p_sketch_0.1_ = c()
for(i in 1:1000){
  xyz <- how_many2(Data = cbind(Szen3w_20k[[i]]$x, Szen3w_20k[[i]]$y), 
                   res_number = 20001,abs = F,
                   w_RW = 2000, R = 500,
                   w_SW = 2000,
                   eps = 0.5)
  xyz2 <- how_many2(Data = cbind(Szen3w_20k[[i]]$x, Szen3w_20k[[i]]$y),
                    res_number = 20001,abs = F,
                    whole = F, RW = F, SW = F, corr_ = F, sketch = T, eps = 0.2)
  xyz3 <- how_many2(Data = cbind(Szen3w_20k[[i]]$x, Szen3w_20k[[i]]$y), 
                    res_number = 20001,abs = F,
  ...
  save(selected_3w_20k_not_abs, file = 'results/3w/selected_3w_20k_not_abs.RData')
  print(i)
}