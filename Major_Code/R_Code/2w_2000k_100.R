...
# Read in the data:
load('../Data/Szen2w_2000k_100.RData')
...
p_sketch_0.1_ = c()
for(i in 1:10){
  xyz <- how_many2(Data = cbind(Szen2w_2000k[[i]]$x, Szen2w_2000k[[i]]$y), 
                   res_number = 2000001,abs = F,
                   whole = F, 
                   w_RW = 2000, R = 2000,
                   w_SW = 2000,
                   eps = 0.5)
  xyz2 <- how_many2(Data = cbind(Szen2w_2000k[[i]]$x, Szen2w_2000k[[i]]$y), 
                    res_number = 2000001,abs = F,
                    whole = F, RW = F, SW = F, corr_ = F, sketch = T, eps = 0.2)
  xyz3 <- how_many2(Data = cbind(Szen2w_2000k[[i]]$x, Szen2w_2000k[[i]]$y),
                    res_number = 2000001,abs = F,
                    whole = F, RW = F, SW = F, corr_ = F, sketch = T, eps = 0.1)
  # Just consider the SNPs with the 10000 largest CLS
  # p_whole_ cannot be calculated here
  ...
  selected_2w_2000k_100_not_abs = list('whole' = NA,
                                  ...
                                  'e_0.2' = p_sketch_0.2_,
                                  'e_0.1' = p_sketch_0.1_)
  save(selected_2w_2000k_100_not_abs, 
       file = 'results/2w/selected_2w_2000k_100_not_abs_neu.RData')
  print(i)
