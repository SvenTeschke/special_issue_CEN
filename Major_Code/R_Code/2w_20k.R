...
p_sketch_0.1_ = c()
for(i in 1:1000){
  xyz <- how_many2(Data = cbind(Szen2w_20k[[i]]$x, Szen2w_20k[[i]]$y), res_number = 20001,
                   abs = F,
                   w_RW = 2000, R = 500,
                   w_SW = 2000,
                   eps = 0.5)
  xyz2 <- how_many2(Data = cbind(Szen2w_20k[[i]]$x, Szen2w_20k[[i]]$y), 
                    res_number = 20001,abs = F,
                    whole = F, RW = F, SW = F, corr_ = F, sketch = T, eps = 0.2)
  xyz3 <- how_many2(Data = cbind(Szen2w_20k[[i]]$x, Szen2w_20k[[i]]$y), 
                    res_number = 20001,abs = F,
  ...
  selected_2w_20k_not_abs = list('whole' = p_whole_,
                                  ...
                                  'e_0.2' = p_sketch_0.2_,
                                  'e_0.1' = p_sketch_0.1_)
  save(selected_2w_20k_not_abs, file = 'results/2w/selected_2w_20k_not_abs_neu.RData')
  print(i)
