...
p_sketch_0.1_ = c()
for(i in 1:100){
  xyz <- how_many2(Data = cbind(Szen4w_200k[[i]]$x, Szen4w_200k[[i]]$y), 
                   res_number = 200001,abs = F,
                   w_RW = 2000, R = 1000,
                   w_SW = 2000,
                   eps = 0.5)
  xyz2 <- how_many2(Data = cbind(Szen4w_200k[[i]]$x, Szen4w_200k[[i]]$y), 
                    res_number = 200001,abs = F,
                    whole = F, RW = F, SW = F, corr_ = F, sketch = T, eps = 0.2)
  xyz3 <- how_many2(Data = cbind(Szen4w_200k[[i]]$x, Szen4w_200k[[i]]$y), 
                    res_number = 200001,abs = F,
  ...
  Szen4w_200k[[i]] <- 'done'
  gc()
  save(selected_4w_200k_not_abs, file = 'results/4w/selected_4w_200k_not_abs.RData')
  print(i)
}