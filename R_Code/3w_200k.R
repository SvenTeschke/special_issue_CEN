...
p_sketch_0.1_ = c()
for(i in 1:100){
  xyz <- how_many2(Data = cbind(Szen3w_200k[[i]]$x, Szen3w_200k[[i]]$y), res_number = 200001,abs = F,
                   w_RW = 2000, R = 1000,
                   w_SW = 2000,
                   eps = 0.5)
  xyz2 <- how_many2(Data = cbind(Szen3w_200k[[i]]$x, Szen3w_200k[[i]]$y), res_number = 200001,abs = F,
                    whole = F, RW = F, SW = F, corr_ = F, sketch = T, eps = 0.2)
  xyz3 <- how_many2(Data = cbind(Szen3w_200k[[i]]$x, Szen3w_200k[[i]]$y), res_number = 200001,abs = F,
  ...
  selected_3w_200k_not_abs = list("whole" = p_whole_,
                                    "RW" = p_RW_,
                                    "SW" = p_SW_,
                                    "corr" = p_corr_,
                                    "e_0.5" = p_sketch_0.5_,
  ...
  Szen3w_200k[[i]] <- "done"
  gc()
  save(selected_3w_200k_not_abs, file = "results/selected_3w_200k_not_abs.RData")
  print(i)
}