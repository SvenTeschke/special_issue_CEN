...
p_sketch_0.1_ = c()
for(i in 1:1000){
  xyz <- how_many2(Data = cbind(Szen2w2w_2k[[i]]$x, Szen2w2w_2k[[i]]$y), res_number = 2001,abs = F,
                   w_RW = 200, R = 200,
                   w_SW = 200,
                   eps = 0.5)
  xyz2 <- how_many2(Data = cbind(Szen2w2w_2k[[i]]$x, Szen2w2w_2k[[i]]$y), res_number = 2001,abs = F,
                    whole = F, RW = F, SW = F, corr_ = F, sketch = T, eps = 0.2)
  xyz3 <- how_many2(Data = cbind(Szen2w2w_2k[[i]]$x, Szen2w2w_2k[[i]]$y), res_number = 2001,abs = F,
