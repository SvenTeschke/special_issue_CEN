...
p_sketch_0.1_ = c()
for(i in 1:1000){
  xyz <- how_many2(Data = cbind(Szen2w_2k[[i]]$x, Szen2w_2k[[i]]$y), res_number = 2001,abs = F,
                   w_RW = 200, R = 200,
                   w_SW = 200,
                   eps = 0.5)
  xyz2 <- how_many2(Data = cbind(Szen2w_2k[[i]]$x, Szen2w_2k[[i]]$y), res_number = 2001,abs = F,
                    whole = F, RW = F, SW = F, corr_ = F, sketch = T, eps = 0.2)
  xyz3 <- how_many2(Data = cbind(Szen2w_2k[[i]]$x, Szen2w_2k[[i]]$y), res_number = 2001,abs = F,
  ...
  selected_2w_2k_not_abs = list("whole" = p_whole_,
                                  "RW" = p_RW_,
                                  "SW" = p_SW_,
                                  "corr" = p_corr_,
                                  "e_0.5" = p_sketch_0.5_,
                                  "e_0.2" = p_sketch_0.2_,
                                  "e_0.1" = p_sketch_0.1_)
  #save(selected_2w_2k_not_abs, file = "selected_2w_2k_not_abs_neu.RData")
  print(i)
}

save(selected_2w_2k_not_abs, file = "results/selected_2w_2k_not_abs_neu.RData")